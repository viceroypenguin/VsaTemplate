...
[Immediate.Cache](https://github.com/immediateplatform/immediate.cache) is used to simplify and automate the
implementation of caching for various IH handlers.
...
* It looks for any classes marked with `[ConfigureOptions]` and generates a method which will automatically configure
  them as options in the DI system.
...
* (Web only) VSA0001: Api Handler must support authorization by implementing `IAuthorizedRequest`.
  * This ensures that all api endpoints are properly protected by an authorization check.
...
* The `Infrastructure` folder contains code to support the infrastructure of the application. Things like `Authentication`, 
  `Authorization`, `Exceptions`, etc. can be found here. In general, no business logic of any kind should be found here.
...
This template is designed primarily as a reading template and guide for implementing a theoretical VSA project. It is a
runnable project as currently written, but many infrastructure elements are minimal, at best; for example, more unit
tests should generally be added for confidence in the completed application.

To run this project, do the following:

1. Clone the project
2. Add a `Secrets.targets` file to the `Web`/`Api` project. This file will contain the following (note that `;` are replaced with `%3B`
...
> [!NOTE]
> The username and password for this file specifically should have permission to create and drop databases. This is
> necessary because a temporary database is created, against which all of the migration scripts are run. The final
> database is used as the source for the schema scaffolding.
...
4. Add a `secrets.json` file to the `Web`/`Api` project. This file will contain the following:
...
> [!NOTE]
> The connection string for this file should be different than for `Scaffold.targets`, and should point specifically to
