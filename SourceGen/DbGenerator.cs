...
public sealed partial class DbGenerator : IIncrementalGenerator
{
	...
	public static (string ColumnName, string TypeName, string UnderlyingTypeName)? TransformVogenValueObject(GeneratorAttributeSyntaxContext context, CancellationToken token)
	{
	...
	private static (string ColumnName, string TypeName, string UnderlyingTypeName) TransformSyncEnum(SyncEnum @enum, CancellationToken token) =>
		(
			@enum.Name + "Id",
			@enum.TypeName,
			"int"
