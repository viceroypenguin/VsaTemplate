...
)
{
	...
	public async ValueTask<bool> IsAuthorized(string policy, IAuthorizationRequirement? requirement)
	{
		if (requirement is null)
			return await IsAuthorized(policy);

		if (await GetCurrentUser() is not { } user)
			return false;

		var auth = await authorizationService.AuthorizeAsync(user, policy, [requirement]);
		return auth.Succeeded;
	}
}
